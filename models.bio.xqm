xquery version "3.0";
module namespace bio.models.bio = "bio.models.bio";
(:~
...
declare default function namespace "bio.models.bio" ;
...
(:~
 ...
 :)
declare function bio.models.bio:mime-type($name as xs:string) as xs:string {
...
declare function getModels($content as map(*)){
  ...
  for $model at $i in $models return
    ...
    then (
      ...
      (:modify replace value of node $doc/xf:model/xf:instance[@id=fn:substring-before($model, 'Model.xml')]/@src with '/bio/' || $path || '/' || $instances[$i]:)
      (:modification avec le passage vers EAC V2:)
      modify replace value of node $doc/xf:model/xf:instance[1]/@src with '/bio/' || $path || '/' || $instances[$i]
      return $doc
    )
    else
